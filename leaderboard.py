...
_visible = False  # Track whether the board is currently shown
...
def setup_leaderboard(player_id):
    ...
    # Start hidden; visibility will be controlled by TAB
    set_visible(False)
    update_leaderboard()
...
def set_visible(visible):
    ...
    global leaderboard_panel, leaderboard_title, leaderboard_entries, _visible
    _visible = visible
...
def update_visibility():
    ...
    global _visible
    # held_keys comes from ursina; True while the key is pressed
    should_show = bool(held_keys.get('tab')) if hasattr(held_keys, "get") else held_keys['tab']
